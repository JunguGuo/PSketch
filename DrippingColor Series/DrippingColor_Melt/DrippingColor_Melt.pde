...
void setup() {
  ...
  cy = height/2;
  for(int i=0; i<num; i++) {
    inkArray[i] = new PVector(random(-300, 300) + cx, random(-290, 290) + cy);
    maxSize[i] = height;
    loops[i] = 0;
...
void keyPressed() {
  //pause = !pause;
    for(int i=0; i<num; i++) {
    inkArray[i] = new PVector(random(-300, 300) + cx, random(count*20, cy));
    colors[i]= backgroundImage.get(int(inkArray[i].x),int(inkArray[i].y));
  }
  count++;
...
void draw() {
  ...
  for(int i=0; i<num; i++) {
    ...
    w =  abs(maxSize[i] / (maxSize[i] - inkArray[i].y));
